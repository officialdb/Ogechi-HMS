<x-admin-layout title="Edit Patient">
<div class="max-w-4xl mx-auto space-y-5"
    ...
    <div class="flex items-center gap-2">
        ...
        <x-fas-tachometer-alt class="w-3 h-3 text-slate-300" />
        <span class="text-xs text-blue-600 font-semibold">Update Patient</span>
    </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input name="first_name" type="text" value="{{ old('first_name', $patient->first_name) }}" required placeholder="e.g. Amaka"
                    ...
                    <div>
                        ...
                        <input name="middle_name" type="text" value="{{ old('middle_name', $patient->middle_name) }}" placeholder="Optional"
                    ...
                    <div>
                        ...
                        <input name="last_name" type="text" value="{{ old('last_name', $patient->last_name) }}" required placeholder="e.g. Okafor"
                    ...
                    <div>
                        ...
                        <input name="date_of_birth" type="date" value="{{ old('date_of_birth', $patient->date_of_birth?->format('Y-m-d')) }}" required max="{{ date('Y-m-d') }}"
                    ...
                    <div>
                        ...
                        <select name="gender" required class="w-full px-4 py-2.5 text-sm border {{ $errors->has('gender') ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-slate-50' }} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors text-slate-700">
                            ...
                            @foreach($selectOptions['genders'] as $value => $label)
                                <option value="{{ $value }}" {{ old('gender', $patient->gender) === $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="marital_status" class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors text-slate-700">
                            ...
                            @foreach($selectOptions['maritalStatuses'] as $value => $label)
                                <option value="{{ $value }}" {{ old('marital_status', $patient->marital_status) === $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input name="phone" type="tel" value="{{ old('phone', $patient->phone) }}" required placeholder="+234 800 000 0000"
                    ...
                    <div>
                        ...
                        <input name="email" type="email" value="{{ old('email', $patient->email) }}" placeholder="patient@email.com"
                               class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                    </div>
                    ...
                    <div class="sm:col-span-2 lg:col-span-3">
                        ...
                        <input name="address" type="text" value="{{ old('address', $patient->address) }}" placeholder="Street address"
                               class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                    </div>
                    ...
                    <div>
                        ...
                        <input name="city" type="text" value="{{ old('city', $patient->city) }}" placeholder="e.g. Enugu"
                               class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">State</label>
                        <input name="state" type="text" value="{{ old('state', $patient->state) }}" placeholder="e.g. Enugu State"
                               class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Country</label>
                        <input name="country" type="text" value="{{ old('country', $patient->country ?? 'Nigeria') }}" placeholder="Nigeria"
                               class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                    </div>
                </div>
            </div>
                ...
                <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="blood_group" class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors text-slate-700">
                            ...
                            @foreach($selectOptions['bloodGroups'] as $value => $label)
                                <option value="{{ $value }}" {{ old('blood_group', $patient->blood_group) === $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="genotype" class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors text-slate-700">
                            ...
                            @foreach($selectOptions['genotypes'] as $value => $label)
                                <option value="{{ $value }}" {{ old('genotype', $patient->genotype) === $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    @can('doctors.create')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="allergies" rows="3" placeholder="List any known allergies (medications, foods, environmental)…"
                                  class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors resize-none placeholder-slate-400">{{ old('allergies', $patient->allergies) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input name="next_of_kin_name" type="text" value="{{ old('next_of_kin_name', $patient->next_of_kin_name) }}" placeholder="Next of kin name"
                                   class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1.5">Phone</label>
                            <input name="next_of_kin_phone" type="tel" value="{{ old('next_of_kin_phone', $patient->next_of_kin_phone) }}" placeholder="+234 800 000 0000"
                                   class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1.5">Relationship</label>
                            <input name="next_of_kin_relationship" type="text" value="{{ old('next_of_kin_relationship', $patient->next_of_kin_relationship) }}" placeholder="e.g. Spouse"
                                   class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1.5">Full Name</label>
                            <input name="emergency_contact_name" type="text" value="{{ old('emergency_contact_name', $patient->emergency_contact_name) }}" placeholder="Emergency contact name"
                                   class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1.5">Phone</label>
                            <input name="emergency_contact_phone" type="tel" value="{{ old('emergency_contact_phone', $patient->emergency_contact_phone) }}" placeholder="+234 800 000 0000"
                                   class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors placeholder-slate-400">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    ...
                    <textarea name="notes" rows="4" placeholder="Any additional clinical notes, pre-existing conditions, or special requirements…"
                              class="w-full px-4 py-2.5 text-sm border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-colors resize-none placeholder-slate-400">{{ old('notes', $patient->notes) }}</textarea>
                </div>
            </div>
