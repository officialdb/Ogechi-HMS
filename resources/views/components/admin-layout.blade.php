...
<div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
           class="fixed top-0 left-0 z-40 h-full w-[230px] bg-white flex flex-col transition-transform duration-300 ease-out lg:static lg:translate-x-0 border-r border-slate-100 shadow-xl lg:shadow-none">

        {{-- Logo --}}
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-5 py-5 border-b border-slate-100 flex-shrink-0">
            <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 shadow-md" style="background:linear-gradient(135deg,#0B5ED7,#1D4ED8);">
            ...
            <div class="leading-tight">
                <p class="text-sm font-bold text-slate-900">Ogechi HMS</p>
                <p class="text-[11px] text-slate-400">Hospital Management</p>
            </div>
        </a>
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            @php
                $navGroups = [
                    'MAIN' => [
                        ...
                        ['label'=>'Doctors',      'route'=>'website.doctors', 'active'=>'website.doctors', 'icon'=>'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'],
                        ['label'=>'Departments',  'route'=>'website.services','active'=>'website.services','icon'=>'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4'],
                        ['label'=>'Appointments', 'route'=>'dashboard',       'active'=>'appointments.*',  'icon'=>'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z','badge'=>'5'],
                        ['label'=>'Pharmacy',     'route'=>'dashboard',       'active'=>'pharmacy.*',      'icon'=>'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2'],
                        ['label'=>'Billing',      'route'=>'dashboard',       'active'=>'billing.*',       'icon'=>'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2z'],
                    ],
                    'SYSTEM' => [
                        ['label'=>'Reports',  'route'=>'dashboard',    'active'=>'reports.*', 'icon'=>'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
                        ['label'=>'Notice',   'route'=>'dashboard',    'active'=>'notice.*',  'icon'=>'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9'],
                        ['label'=>'Settings', 'route'=>'profile.edit', 'active'=>'profile.*', 'icon'=>'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z'],
                    ],
                ];
            @endphp
            ...
            @foreach($navGroups as $groupLabel => $items)
                <div class="{{ $loop->first ? '' : 'pt-3' }}">
                    <p class="px-4 mb-1.5 text-[10px] font-bold tracking-[0.12em] text-slate-400 uppercase">{{ $groupLabel }}</p>
                    @foreach($items as $item)
                        @php $isActive = request()->routeIs($item['active']); @endphp
                        <a href="{{ route($item['route']) }}"
                           class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 mb-0.5 {{ $isActive ? 'nav-item-active' : 'nav-item-inactive' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-[18px] h-[18px] flex-shrink-0 {{ $isActive ? 'text-white' : 'text-slate-400' }}"
                            ...
                            @if(!empty($item['badge']))
                                <span class="text-[10px] font-bold px-1.5 py-0.5 rounded-full {{ $isActive ? 'bg-white/25 text-white' : 'bg-blue-100 text-blue-600' }}">{{ $item['badge'] }}</span>
                            @endif
                        </a>
                    @endforeach
                </div>
        ...
        {{-- Create New --}}
        <div class="m-3 mb-4 rounded-2xl p-4 flex-shrink-0" style="background:linear-gradient(135deg,#EFF6FF,#DBEAFE);border:1px solid #BFDBFE;">
            ...
            <a href="{{ route('patients.create') }}" class="block w-full text-center text-white text-xs font-bold py-2 rounded-xl transition-all duration-200 hover:opacity-90 shadow-md" style="background:linear-gradient(135deg,#0B5ED7,#1D4ED8);">
                + Create New
            </a>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {{-- Header --}}
        <header class="bg-white border-b border-slate-100 px-6 py-3.5 flex items-center gap-4 flex-shrink-0 z-20 shadow-sm">
            ...
            <div class="flex items-center gap-2.5 ml-auto">
                <button class="relative w-9 h-9 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-colors">
