...
<div
    x-data="{
        sidebarOpen: false,
        loading: true,
        init() { setTimeout(() => this.loading = false, 1800) }
    }"
...
>
    ...
    <aside
        ...
        class="fixed top-0 left-0 z-40 h-full w-[230px] bg-white flex flex-col
    ...
    >
        ...
        <a href="{{ route('dashboard') }}"
           class="flex items-center gap-3 px-5 py-5 border-b border-slate-100 flex-shrink-0">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            ...
            @foreach($navGroups as $groupLabel => $items)
                <div class="{{ $loop->first ? '' : 'pt-3' }}">
                    <p class="px-4 mb-1.5 text-[10px] font-bold tracking-[0.12em] text-slate-400 uppercase">
                        {{ $groupLabel }}
                    </p>
                    @foreach($items as $item)
                        @php $isActive = request()->routeIs($item['active']); @endphp
                        <a href="{{ route($item['route']) }}"
                           class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 mb-0.5
                                  {{ $isActive ? 'nav-item-active' : 'nav-item-inactive' }}">
                            <svg xmlns="http://www.w3.org/2000/svg"
                                 class="w-[18px] h-[18px] flex-shrink-0 {{ $isActive ? 'text-white' : 'text-slate-400' }}"
        ...
        {{-- Create New card --}}
        <div class="m-3 mb-4 rounded-2xl p-4 flex-shrink-0"
            ...
            <a href="{{ route('patients.create') }}"
               class="block w-full text-center text-white text-xs font-bold py-2 rounded-xl transition-all duration-200 hover:opacity-90 hover:shadow-md"
               style="background: linear-gradient(135deg,#0B5ED7,#1D4ED8);">
                + Create New
            </a>
        </div>
