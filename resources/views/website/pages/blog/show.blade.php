    ...
    <section class="relative py-20 lg:py-28 overflow-hidden" style="background: linear-gradient(135deg, #062C77 0%, #0B5ED7 60%, #1565C8 100%);">
        ...
        <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div class="flex items-center justify-center gap-4 text-sm">
                ...
                <time datetime="{{ $post['date_iso'] }}" class="text-blue-200 text-xs flex items-center gap-1">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <article class="lg:col-span-2">
                    {{-- Featured banner --}}
                    <div class="h-64 rounded-2xl bg-gradient-to-br {{ $post['grad'] }} mb-8 relative overflow-hidden flex items-center justify-center">
                        ...
                        <div class="absolute bottom-4 left-4 flex gap-2">
                            <span class="{{ $post['cat_color'] }} text-xs font-bold px-3 py-1 rounded-full bg-white shadow">{{ $post['category'] }}</span>
                        </div>
                    </div>
                ...
                <aside class="space-y-6">
                    ...
                    <div class="bg-[#F5F9FF] rounded-2xl p-5 border border-blue-50">
                        ...
                        <a href="{{ route('website.doctors') }}" class="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-800 transition-colors">
                            View Profile <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                        </a>
                    </div>
                        ...
                        <div class="space-y-4">
                            @foreach($related as $rel)
                                <a href="{{ route('website.blog.show', $rel['slug']) }}" class="flex gap-3 group">
                                    <div class="w-16 h-16 rounded-xl bg-gradient-to-br {{ $rel['grad'] }} flex-shrink-0 flex items-center justify-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="{{ $rel['icon_path'] }}"/>
                                        </svg>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-blue-600 font-medium mt-1">{{ $rel['date'] }}</p>
                                    </div>
                                </a>
                            @endforeach
                        </div>
                    ...
                    <div class="bg-[#F5F9FF] rounded-2xl p-5 border border-blue-50">
                        ...
                        <div class="space-y-2">
                            @foreach(['Cardiology','Neurology','Orthopedics','Dentistry','Pediatrics','Emergency Care'] as $dept)
